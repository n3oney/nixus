...
in {
  options.services.binary-cache.server.enable = lib.mkEnableOption "harmonia binary cache server";

  config = lib.mkMerge [
    (lib.mkIf cfg.server.enable {
      os.services.harmonia.cache = {
        enable = true;
        signKeyPaths = [osConfig.age.secrets."binary-cache-${thisHost}".path];
        settings.bind = "[::]:5000";
      };
